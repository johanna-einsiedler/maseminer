...
```bash
git clone https://github.com/johanna-einsiedler/maseminer.git
cd maseminer
...
You are reading **MASEMiner version `v1.1.0`**.  The same string lives in the `VERSION` file at the repo root — cite that exact string for reproducibility.

```
Einsiedler, J. (2026). MASEMiner (version v1.1.0) [Computer software].
https://github.com/johanna-einsiedler/maseminer
```
